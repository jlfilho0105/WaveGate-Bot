...
RESPONSABILIDADE: Rastreia posições abertas, equity, P&L e persiste estado em JSON.
"""
...
class PortfolioAgent:
    def __init__(self, config: dict):
        ...
        self.open_positions:  dict = {}   # {symbol: {"signal": TradeSignal, ...}}
    ...
    def open_position(self, signal: TradeSignal) -> None:
        ...
        self._save_state()
        logger.info(
            f"Posição aberta: {signal.symbol} | "
            f"entry={signal.entry_price:.6f} | "
            f"margin={signal.position_size_usdt:.2f} USDT"
        )
    ...
    def close_position(self, symbol: str, exit_price: float, outcome: str) -> float:
        ...
        else:  # TIMEOUT — usa preço de saída
            pnl = margin * signal.leverage * (
        ...
        self._save_state()
        logger.info(
            f"Posição fechada: {symbol} | {outcome} | "
    ...
    def summary(self) -> str:
        ...
        return (
            ...
            f"Abertas: {len(self.open_positions)} | Exposição: {self.open_exposure_usdt:.2f} USDT"
        )
